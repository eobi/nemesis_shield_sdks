...
class SentinelWSGI:
    ...
    def _info(self, environ):
        ...
        return method, full, authed

    def __call__(self, environ, start_response):
        method, full, authed = self._info(environ)

        # enforce: block off-baseline shapes before dispatch
        if self.client.mode == "enforce":
            verdict = self.client.decide(build_sketch(method=method, path=full, authenticated=authed, status=0))
            if self.client.should_block(verdict):
                try:
                    self.client.record(build_sketch(method=method, path=full, authenticated=authed, status=403))
                except Exception:
                    pass
        ...
        try:
            self.client.record(build_sketch(method=method, path=full, authenticated=authed, status=captured["status"]))
        except Exception:
            pass  # fail-open
        return result
