...
const TOPICS: Record<string, string> = {
  ...
  screening:
    "Omniguard verification & screening is a STANDALONE API — no scoring function needed. One call verifies an identity (BVN / NIN / Passport), screens a name against sanctions & PEP watchlists, runs an adverse-media check, or checks an email/domain for breach exposure. Use it at customer onboarding (KYC/KYB) and before moving money. It's honest by construction: a check with no provider connected returns pending/failed with a reason, never a fabricated pass; every result is written to the Verifications history and metered by the plan's allowance. From an agent, call nemesis_omniguard_verify with the Omniguard ingest token.",
  ...
  aml: "See 'screening' and 'business-logic' — Omniguard covers AML two ways: real-time transaction scoring for structuring/layering/mule typologies (nemesis_omniguard_score), and standalone sanctions & PEP screening plus adverse-media at onboarding (nemesis_omniguard_verify). Flagged activity carries an AML case trail with regulatory reporting (goAML/NFIU) in the console.",
  magecart:
